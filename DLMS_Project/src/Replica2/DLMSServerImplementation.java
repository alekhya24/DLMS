...
public class DLMSServerImplementation {
	...
	public DLMSServerImplementation(Servers libraryLocation) {
		super();
        ...
        for (Servers loc : Servers.values()) {
            if (loc.toString()== this.location) {
                ...
                try {
                	...
                	req[counter] = new UDPRequestProvider(Replica2.serverData.get(loc.toString()),data);
                } catch (IOException e) {
	...
	private void assignWaitlistingMembers(String itemID) 
	{
		...
		else
		{
			 ...
			 try {
				 ...
				 Queue<String> waitingItemsList= Replica2.serverData.get(loc.toString()).waitingListMap.get(itemID);
